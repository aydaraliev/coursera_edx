...
samp1 %>%
  summarise(mean = mean(area), med = median(area), 
...
sample_means50 %>%
  summarise(mean = mean(x_bar), med = median(x_bar), 
            s = sd(x_bar), pop_iqr = IQR(x_bar),
...
length(sample_means_small$x_bar)
...
sample_50_price = ames %>% sample_n(size = 50) %>% summarise(mean(price))
sample_50_price
...
sample_means15_price <- ames %>%
                     rep_sample_n(size = 15, reps = 5000, replace = TRUE) %>%
                     summarise(x_bar = mean(price))
...
mean(sample_means15_price$x_bar)
```
...
mean(sample_means150_price$x_bar)
