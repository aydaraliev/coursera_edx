...
inference(y = weight, x = habit, data = nc, statistic = "mean", type = "ht", conf_level = 0.95, null = 0, 
...
#confidence interval became a little bit narrower
...
```{r gained-mature-ht}
inference(y = gained, x = mature, data = nc, statistic = "mean", type = "ht", conf_level = 0.95, null = 0, order = c("younger mom", "mature mom"),
...
```{r cutoff-mature}
ggplot(nc, aes(mature, mage)) + geom_boxplot()
min(filter(nc, nc$mature == "mature mom")$mage)
max(filter(nc, nc$mature == "younger mom")$mage)
...
```{r pick-your-own}
t.test(nc$fage, nc$mage)
#is father age different from mother age?
#given the p-value we can reject the null hypothesis of fage-mage = 0, difference in
#means does not equal to zero.
#We are 95% confident that the true difference lies between 2.655048 and 3.856411
```
