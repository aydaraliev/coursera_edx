...
```{r two-vars-eda}
ggplot(evals, aes(gender, age)) + geom_boxplot()
...
ggplot(data = lm_bty, aes(x = .fitted, y= .resid)) + geom_point() +
...
```{r diag-mlr-model}
ggplot(data = m_bty_gen, aes(x = .fitted, y= .resid)) + geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed")
ggplot(data = m_bty_gen, aes(sample = .resid)) + geom_qq()
