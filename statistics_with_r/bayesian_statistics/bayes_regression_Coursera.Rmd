...
```{r wage-dist}
ggplot(wage, aes(x = wage)) + geom_histogram() + geom_vline(xintercept = mean(wage$wage))
Mode <- function(x) {
  ux <- unique(x)
...
Mode(wage$wage)
n <- nrow(wage)
wage %>% summarize(Min = min(wage), Q25 = quantile(wage, 0.25), Median = median(wage), Mean = mean(wage), Q75 = quantile(wage, 0.75), Max = max(wage), Less300 = quantile(wage, 7/n))
```
...
```{r two-vars-eda}
ggplot(wage, aes(exper, wage)) + geom_point()
ggplot(wage, aes(educ, wage)) + geom_point()
...
hist(m_wage_iq$residuals, plot = TRUE)
```
...
hist(m_wage_ed$residuals, plot = TRUE)
...
```{r log-resid}
hist(m_lwage_iq$residuals, plot = TRUE)
```
...
```{r z-iq}
m_lwage_liq <- lm(lwage ~ log(iq), data = wage)
summary(m_lwage_liq)
sd <- summary(m_lwage_liq)$sigma
beta_liq <- summary(m_lwage_liq)$coefficients[2,1]
wage_increase_by_1sd <- beta_liq*sd; print(wage_increase_by_1sd)
...
```{r}
beta_m_lwage_full_contant <- summary(m_lwage_full)$coefficients[1,1]
beta_m_lwage_full_married <- summary(m_lwage_full)$coefficients[9,1]
beta_m_lwage_full_black <- summary(m_lwage_full)$coefficients[10,1]
wage_married_black <- beta_m_lwage_full_contant + beta_m_lwage_full_black * 1 + beta_m_lwage_full_married * 1
# 1 each for black and married #
wage_single_nonblack <- beta_m_lwage_full_contant + beta_m_lwage_full_black * 0 + beta_m_lwage_full_married * 0
# 0 each for nonblack and non-married #
data.frame(wage_married_black, wage_single_nonblack)
```
...
```{r}
m_lwage_nosib <- lm(lwage ~ . -wage -sibs, data = na.omit(wage))
m_lwage_nofeduc <- lm(lwage ~ . -wage -feduc, data = na.omit(wage))
m_lwage_nomeduc <- lm(lwage ~ . -wage -meduc, data = na.omit(wage))
BIC_nobrthord <- BIC(m_lwage_nobrthord)
BIC_nosibs <- BIC(m_lwage_nosib)
BIC_nofeduc <- BIC(m_lwage_nofeduc)
BIC_nomeduc <- BIC(m_lwage_nomeduc)
data.frame(BIC_nobrthord, BIC_nosibs, BIC_nofeduc, BIC_nomeduc)
...
```{r stepBIC}
m_lwage_full1 <- lm(lwage ~ . -wage, data = na.omit(wage))
m_lwage_full1_step <- stepAIC(m_lwage_full1, trace = FALSE, direction = "backward")
m_lwage_full1_step$anova
```
...
bma_lwage_red <- bas.lm(lwage ~ . -wage, data = wage_red_no.na,
                        prior = "BIC",
                        modelprior = uniform())
bma_lwage_red
...
```{r}
zs_lwage_red <- bas.lm(lwage ~ . -wage, data = wage_red_no.na,
                       prior = "ZS-null",
                       modelprior = beta.binomial(1,1))

zs_lwage_red

sort(summary(zs_lwage_red)[1:12, 1], decreasing = FALSE)
```
...
```{r}
L <- quantile(y_5, 0.025)
U <- quantile(y_5, 0.975)
c(L, U)
...
ci_bpm_lwage_red <- confint(BPM_pred_lwage_red, estimator = "BPM")
opt_bpm <- which.max(BPM_pred_lwage_red$fit)
exp(ci_bpm_lwage_red[opt_bpm,])
```
