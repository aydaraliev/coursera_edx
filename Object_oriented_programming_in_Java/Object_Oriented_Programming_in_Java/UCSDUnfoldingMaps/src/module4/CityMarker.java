...
public class CityMarker extends SimplePointMarker {
	...
	public void draw(PGraphics pg, float x, float y) {
		...
		triangleWrapper(pg, x, y, 15);
		
		// Restore previous drawing style
		pg.popStyle();
	}
	
	private void triangleWrapper(PGraphics pg, float x, float y, float size) {
		pg.fill(255,0,0);
		pg.triangle(x, y+((float)Math.sqrt(3)/3*size), 
				x-(size/2), y-((float)Math.sqrt(3)/3*size), 
				x+(size/2), y-((float)Math.sqrt(3)/3*size));
	}
