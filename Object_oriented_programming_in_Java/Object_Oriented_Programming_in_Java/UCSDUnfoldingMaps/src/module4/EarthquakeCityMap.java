...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		HashMap<String, Integer> country_eq_count = new HashMap<String, Integer>();
		
		int ocean_markers = quakeMarkers.size();
		
		for (Marker eq : quakeMarkers) {
			if (eq instanceof LandQuakeMarker) {
				ocean_markers--;
				String country = (String)eq.getProperty("country");
				country_eq_count.put(country, country_eq_count.getOrDefault(country, 0)+1);
			}
		}
		country_eq_count.put("OCEAN_QUAKES", ocean_markers);
		for (String country : country_eq_count.keySet()) {
			System.out.println(country + " " + country_eq_count.get(country));
		}
	}
