...
public class ParseFeed {
	...
	public static HashMap<String, Float> loadSalariesFromCSV(PApplet p, String fileName) {
		HashMap<String, Float> salaryHMap = new HashMap<String, Float>();
		String[] rows = p.loadStrings(fileName);
		for (int row = 6; row < rows.length; row++) {
			String[] columns = rows[row].split(",(?=([^\"]*\"[^\"]*\")*[^\"]*$)");
			if (!columns[61].equals("\"\"")) {
				salaryHMap.put(columns[1].replaceAll("^\"|\"$", ""), 
						Float.parseFloat(columns[61].replaceAll("^\"|\"$", "")));
			}
//            for(int i = columns.length - 1; i > 3; i--) {
//				if(!columns[i].equals("..")) {
//					salaryHMap.put(columns[3], Float.parseFloat(columns[i]));
//					break;
//				}
//            }
		}
